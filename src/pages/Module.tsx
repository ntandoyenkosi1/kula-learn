...
const Module=(props:any)=>{
    const placeHolders:ModuleData=props.data[0]
    const [data, setData]=useState<ModuleData>(placeHolders)
    ...
    props.data.forEach((l:ModuleData)=>{
        ...
        tabCell.onclick=()=>{console.log(l.title);setData(l)}
    ...
    useEffect(()=>{
        var divContainer = document.getElementById("sidebar");
    ...
    return <div> 
        <div className="w3-sidebar w3-bar-block w3-collapse w3-card w3-animate-left" style={{width:"200px"}} id="mySidebar">
			...
			<button className="w3-bar-item w3-button w3-large w3-hide-large" onClick={()=>document.getElementById("mySidebar")!.style.display = "none"}>Close &times;</button>
            <div style={{justifyContent:"center", alignItems:"center"}} id="sidebar"></div>
		</div>
        <div className="w3-main" style={{marginLeft:"210px"}}>
        <div className="w3-teal">
		<button className="w3-button w3-teal w3-xlarge w3-hide-large" onClick={()=>document.getElementById("mySidebar")!.style.display = "block"}>&#9776;</button>
		<div className="w3-container">
		<Navigation/>
		</div>
		</div>
            <Accordion defaultActiveKey="0">
                <h1 style={{width:"99%"}} className="alert alert-primary">{data?.title}</h1>
                <Accordion.Item eventKey="0" style={{width:"99%"}}>
                <Accordion.Header><h2>Description</h2></Accordion.Header>
                <Accordion.Body>
                    <div className="container">
                        <div className="row">
                            <div style={{textAlign:"justify"}} className="col justify">{data?.shortDescription}</div>
